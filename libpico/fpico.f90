...
contains
    ...
    subroutine Pico_GetResults(P, error)
        ...
        call fpico_set_param("omnh2", p%omegan*(p%H0/100.)**2)
        call fpico_set_param("omvh2", p%omegav*(p%H0/100.)**2)
        call fpico_set_param("omk", p%omegak)
        call fpico_set_param("H0", p%H0)
        call fpico_set_param("w", real(w_lam,8))
        call fpico_set_param("theta", CosmomcTheta())
        call fpico_set_param("Yp", p%yhe)
        call fpico_set_param("Nnu_massless", p%Num_Nu_massless)
        call fpico_set_param("Nnu_massive", real(p%Num_Nu_massive,8))
        call fpico_set_param("Neff", p%Num_Nu_massless + p%Num_Nu_massive) !TODO: get rid of this
        call fpico_set_param("ns",p%InitPower%an(1))
        call fpico_set_param("nt",p%InitPower%ant(1))
        call fpico_set_param("nrun",p%InitPower%n_run(1))
        call fpico_set_param("r",p%InitPower%rat(1))
        call fpico_set_param("logA",log(10.**10*p%InitPower%ScalarPowerAmp(1)))
        call fpico_set_param("k0",p%InitPower%k_0_scalar)
        call fpico_set_param("tau",p%Reion%optical_depth)
